function result = dft2midi(filename, sampleFrame)
if (nargin<2)
...
ampNotaAttack = 0;
tic
while pointer + sampleFrame <= sampleLength
    x = sample(pointer:pointer + sampleFrame);    
    xFreq = dft(x);    
    ampMaior = 0;
    note = 0;
...
writemidi(midi_new, strcat(filename, 'dft.mid'));
